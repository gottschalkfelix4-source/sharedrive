  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-4">
      ...
      <div className="text-center">
        <p className="text-text-primary font-medium">Could not load dashboard data</p>
        <p className="text-text-muted text-sm mt-1">Make sure the backend is running and you're logged in.</p>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-text-muted text-sm mt-1">Overview of your ShareDrive instance</p>
      </div>
        ...
        {isLoading ? (
          ...
          <>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }}>
              <StatCard title="Total users" value={data.totalUsers} icon={<Users size={20} />} color="text-indigo-400" />
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
              <StatCard title="Active transfers" value={data.activeTransfers} icon={<Files size={20} />} color="text-emerald-400" />
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              <StatCard title="Downloads today" value={data.downloadsToday} icon={<Download size={20} />} color="text-sky-400" />
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
              <StatCard title="Storage used" value={formatBytes(data.storageUsedBytes)} icon={<HardDrive size={20} />} color="text-violet-400" />
            </motion.div>
          </>
        )}
      </div>
        ...
        <>
          ...
          <div className="bg-bg-card border border-border rounded-2xl p-6">
            ...
            {data.downloadHistory.every((d) => d.count === 0) ? (
              <div className="h-[200px] flex items-center justify-center text-text-muted text-sm">
                No downloads yet in the last 7 days
              </div>
            ) : (
          ...
          <div className="bg-bg-card border border-border rounded-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              ...
              <Link to="/admin/files" className="text-xs text-primary hover:underline">View all</Link>
            </div>

            {data.recentTransfers.length === 0 ? (
              <div className="px-6 py-10 text-center text-text-muted text-sm">
                No transfers yet. Upload something to get started!
              </div>
            ) : (
                  ...
                  <div key={t.shortId} className="flex items-center gap-4 px-6 py-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-text-muted">
                        {t.uploaderUsername ? `@${t.uploaderUsername}` : 'Anonymous'} · {formatBytes(t.totalSize)} · {t.fileCount} files
                      </p>
                    </div>
